...
export function docsTask(input: DocsTaskInput): string {
  ...
  return [
    ...
    '',
    'Finish with a report as your last message. Start it with the page URL on',
    'its own labelled line, so nobody has to guess which link is which:',
    '',
    '  Page: <the URL of the page you wrote>',
    '',
    'Then: which sections',
...
export function docsDelegation(
  ...
  return {
    ...
    ...workerRun(config, DOCS_TIMEOUT_MS),
