...
5. Write one entry per case into the results file named in this delegation, holding:
   ...
   - The screenshot filename, saved beside the results file.
   - For a Failed case, what you expected, and one suggestion for the fix. Be specific. The engineer applies your suggestion without re-diagnosing, so a vague one wastes a cycle.
6. Check the accessible name, the focus order, and the contrast on every interactive state.

You write a file. You do not write to the tracking table, and you have no way to. Somebody reads your file, puts the rows in, and attaches your screenshots. That split is deliberate: a tester who could write its own results could write a table full of passes and move the component on by itself, and then nobody's results would be worth reading.
...
- You do not touch the status of anything. You write your results file and you report.
